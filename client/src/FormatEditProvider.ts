...
class FormatEditProvider implements
	...
	private keyWordRegex: RegExp = /^\b(at|configuration|nclob|virtual|cycle|global|row|timestamp|bigint|constraint|foreign|references|column|comment|primary|key|asc|sequence|start|with|by|view|type|library|sqlscript|replace|increment|minvalue|maxvalue|right|public|variable|constant|invoker|true|false|for|do|exit handler|reads|sqlexception|elseif|record_count|round|round_half_up|array_agg|cardinality|string|cast|avg|min|concat|bintostr|sum|(row|table) like|procedure|function|returns?|in|out|declare|using|array|language|sql|security|as|begin|end|if|is|not|or|else|and|then|case|when|while|(tiny)?int|date|integer|boolean|n?varchar|decimal|table|data|select|distinct|top|union all|(inner|left outer) join|on|ifnull|(group|order) by|desc|where|count|into|from|call|break|continue|now|null|values|value|update|set|insert|delete|commit|work|exists|exec)\b$/i;
